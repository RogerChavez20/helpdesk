...
switch ($_GET["op"]) {
    case "guardaryeditar":
        $datos = $usuario->get_usuario_x_correo_excluyendo_id($_POST["usu_correo"], $_POST["usu_id"]);
        if (count($datos) == 0) {
    ...
    case "listar":
        ...
        foreach ($datos as $row) {
            ...
            if ($row["rol_id"] == "1") {
                ...
                $sub_array[] = '<span class="label label-pill label-warning">Helpdesk</span>';
            } else {
    ...
    case "totalXTecnico":
        $datos = $usuario->get_usuario_total_x_id_tecnico($_POST["usu_id"]);
    ...
    case "totalabiertoXTecnico":
        $datos = $usuario->get_usuario_totalabierto_x_id_tecnico($_POST["usu_id"]);
        if (is_array($datos) == true and count($datos) > 0) {
            foreach ($datos as $row) {
                $output["TOTAL"] = $row["TOTAL"];
            }
    ...
    case "totalcerradoXTecnico":
        $datos = $usuario->get_usuario_totalcerrado_x_id_tecnico($_POST["usu_id"]);
        if (is_array($datos) == true and count($datos) > 0) {
            foreach ($datos as $row) {
                $output["TOTAL"] = $row["TOTAL"];
            }
    ...
    case "graficoXTecnico":
        $datos = $usuario->get_usuario_grafico_x_tecnico($_POST["usu_id"]);
        echo json_encode($datos);
        break;
