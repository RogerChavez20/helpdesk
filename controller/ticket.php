...
switch ($_GET["op"]) {
    ...
    // Listar tickets por usuario asignado a resolver
    case "listar_x_sup":
        $datos = $ticket->listar_x_sup($_POST["usu_id"]);
        $data = array();
        foreach ($datos as $row) {
            $sub_array = array();
            $sub_array[] = $row["tick_id"];
                                    ...
                                    <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                                        ...
                                        foreach ($datos_det as $row_det) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="../../public/document_detalle/<?php echo $row_det["tickd_id"]; ?>/<?php echo $row_det["det_nom"]; ?>"
                                                       target="_blank" class="btn btn-inline btn-primary btn-sm">Ver</a>
                                                </td>
                                            </tr>
                                            <?php
                                        }
    ...
    case "mostrar";
        ...
        if (is_array($datos) == true and count($datos) > 0) {
            foreach ($datos as $row) {
                ...
                if ($row["tick_estado"] == "Abierto") {
                    $output["tick_estado"] = '<span class="label label-pill label-success text-uppercase">Abierto</span>';
                } else {
                    $output["tick_estado"] = '<span class="label label-pill label-danger text-uppercase">Cerrado</span>';
                }
                ...
                if ($row["usu_asig"] == null || $row["usu_asig"] == "") {
                    $output["tec_nom"] = "Pendiente";
                    $output["tec_ape"] = "de asignar";
                } else {
                    $tech = new Usuario();
                    $tecnico = $tech->get_usuario_x_id($row["usu_asig"]);
                    if (is_array($tecnico) == true and count($tecnico) > 0) {
                        foreach ($tecnico as $tec) {
                            $output["tec_nom"] = $tec["usu_nom"];
                            $output["tec_ape"] = $tec["usu_ape"];
                        }
                    }
                }
                break;
    ...
    case  "tecnico_calendar":
        $datos = $ticket->get_calendar_tecnico($_POST["usu_id"]);
        echo json_encode($datos);
        break;
}
?>