...
$(document).ready(function(){
    ...
    if ( $('#rol_idx').val() == 1){
        ...
        $.post("../../controller/usuario.php?op=totalXTecnico", {usu_id:usu_id}, function (data) {
            data = JSON.parse(data);
            $('#lbltotal').html(data.TOTAL);
        });

        $.post("../../controller/usuario.php?op=totalabiertoXTecnico", {usu_id:usu_id}, function (data) {
            data = JSON.parse(data);
            $('#lbltotalabierto').html(data.TOTAL);
        });

        $.post("../../controller/usuario.php?op=totalcerradoXTecnico", {usu_id:usu_id}, function (data) {
            data = JSON.parse(data);
            $('#lbltotalcerrado').html(data.TOTAL);
        });

        $.post("../../controller/usuario.php?op=graficoXTecnico", {usu_id:usu_id},function (data) {
            data = JSON.parse(data);
        ...
        $('#idcalendar').fullCalendar({
            ...
            defaultView:'month',
            events:{
                url:'../../controller/ticket.php?op=tecnico_calendar',
                method:'POST',
                data:{usu_id:usu_id}
            }
        });
